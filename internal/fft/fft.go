...
func FFTN(m *dsp.Matrix) *dsp.Matrix {
	return computeFFTN(m, FFT)
}

// IFFTN returns the forward FFT of the matrix m, computed in all N dimensions.
func IFFTN(m *dsp.Matrix) *dsp.Matrix {
	return computeFFTN(m, IFFT)
}

func computeFFTN(m *dsp.Matrix, fftFunc func([]complex64) []complex64) *dsp.Matrix {
	dims := m.Dimensions()
	t := m.Copy()
	r := dsp.MakeEmptyMatrix(dims)
