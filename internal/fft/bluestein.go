...
func getBluesteinFactors(input_len int) ([]complex64, []complex64) {
	...
	if !hasBluesteinFactors(input_len) {
		...
		for i := 0; i < input_len; i++ {
			...
			bluesteinFactors[input_len][i] = complex64(complex(cos, sin))
			bluesteinInvFactors[input_len][i] = complex64(complex(cos, -sin))
		}
	}
...
func bluesteinFFT(x []complex64) []complex64 {
	lx := len(x)
	a := dsp.ZeroPad(x, dsp.NextPowerOf2(lx*2-1))
	la := len(a)
