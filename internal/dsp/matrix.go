...
//     4, 3, 2, 1},
//   []int {2, 3, 4})
func MakeMatrix(x []complex64, dims []int) *Matrix {
	length := 1
...
func MakeMatrix2(x [][]complex64) *Matrix {
	dims := []int{len(x), len(x[0])}
	r := make([]complex64, dims[0]*dims[1])
	for n, v := range x {
		if len(v) != dims[1] {
			panic("ragged array")
		}
...
func (m *Matrix) Copy() *Matrix {
	...
	r.list = make([]complex64, len(m.list))
	copy(r.list, m.list)
	return r
}
...
func MakeEmptyMatrix(dims []int) *Matrix {
	...
	return MakeMatrix(make([]complex64, x), dims)
}
...
func (s *Matrix) Dim(dims []int) []complex64 {
	inds := s.indexes(dims)
	r := make([]complex64, len(inds))
	for n, v := range inds {
		r[n] = s.list[v]
	}
...
func (m *Matrix) SetDim(x []complex64, dims []int) {
	inds := m.indexes(dims)
	if len(x) != len(inds) {
...
func (s *Matrix) Value(dims []int) complex64 {
